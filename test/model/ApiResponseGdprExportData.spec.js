/**
 * SimpleBilly API
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.SimpleBillyApi);
  }
}(this, function(expect, SimpleBillyApi) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new SimpleBillyApi.ApiResponseGdprExportData();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('ApiResponseGdprExportData', function() {
    it('should create an instance of ApiResponseGdprExportData', function() {
      // uncomment below and update the code to test ApiResponseGdprExportData
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be.a(SimpleBillyApi.ApiResponseGdprExportData);
    });

    it('should have the property activityLog (base name: "activityLog")', function() {
      // uncomment below and update the code to test the property activityLog
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property apiKeys (base name: "apiKeys")', function() {
      // uncomment below and update the code to test the property apiKeys
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property billing (base name: "billing")', function() {
      // uncomment below and update the code to test the property billing
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property exportedAt (base name: "exportedAt")', function() {
      // uncomment below and update the code to test the property exportedAt
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property generatedByAi (base name: "generatedByAi")', function() {
      // uncomment below and update the code to test the property generatedByAi
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property notifications (base name: "notifications")', function() {
      // uncomment below and update the code to test the property notifications
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property refreshTokens (base name: "refreshTokens")', function() {
      // uncomment below and update the code to test the property refreshTokens
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property tenants (base name: "tenants")', function() {
      // uncomment below and update the code to test the property tenants
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property usageEvents (base name: "usageEvents")', function() {
      // uncomment below and update the code to test the property usageEvents
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

    it('should have the property user (base name: "user")', function() {
      // uncomment below and update the code to test the property user
      //var instance = new SimpleBillyApi.ApiResponseGdprExportData();
      //expect(instance).to.be();
    });

  });

}));
