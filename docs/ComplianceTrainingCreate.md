# SimpleBillyApi.ComplianceTrainingCreate

## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**assignable** | **Boolean** | Whether HR can assign this training as required for employees. | [optional] 
**code** | **String** | Stable code used by plugins and frontend players (e.g. \&quot;data_privacy\&quot;). | [optional] 
**description** | **String** |  | [optional] 
**passScore** | **Number** | Minimum score (0–100) required to pass. | [optional] 
**pluginPlatform** | **String** | Marketplace plugin platform id when source &#x3D; Plugin. | [optional] 
**source** | [**TrainingSource**](TrainingSource.md) |  | [optional] 
**title** | **String** |  | [optional] 
**validityMonths** | **Number** | Certificate validity in months; null &#x3D; no expiry. | [optional] 


